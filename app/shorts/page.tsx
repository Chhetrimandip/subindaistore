    ...
    const fetchProducts = async () => {
      try {
        const products = await client.fetch(
          `*[_type == "product" && category->slug.current == "shorts"] | order(_createdAt desc) {
            _id,
            name,
            "slug": slug.current,
            price,
    ...
    return (
      <main className="min-h-screen bg-[#F8F9FC] text-slate-900 pt-28 pb-20 px-4 md:px-20 max-w-[1440px] mx-auto">
        <div className="flex items-center justify-center h-64">
          <p className="text-slate-500">Loading shorts...</p>
        </div>
      </main>
    );
  }
    ...
    <main className="min-h-screen bg-[#F8F9FC] text-slate-900 pt-28 pb-20 px-4 md:px-20 max-w-[1440px] mx-auto">
      ...
      <header className="mb-10 max-w-3xl">
        ...
        <p className="text-sm md:text-base text-slate-500 leading-relaxed font-normal">
          Discover our curated collection of authentic, high-performance shorts from elite global leagues. 
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-start">
        ...
        <div className="flex-grow w-full">
          ...
          {displayedProducts.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-64">
              <p className="text-slate-500 text-lg">No shorts found matching your criteria.</p>
            </div>
          )}
        </div>
      </div>
